...
| Aidbox | Not yet verified | No | No | Adapter present, verification wanted | `lib/fhir/aidbox.ts` (basic-auth Aidbox Client against the `/fhir` endpoint) passes the wire-level contract but needs a licensed synthetic box for the real-server contract and Safety Eval before it joins `FHIR_BACKEND`. See the [adapter guide](./adapters.md). |
