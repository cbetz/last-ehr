...
| Oystehr (`FHIR_BACKEND=oystehr`) | Pending verification | No | No | Adapter merged, verification pending | OAuth2 M2M client credentials against Oystehr's hosted FHIR R4 API. The adapter, its unit contract, and the eval wiring are in place; the real-sandbox contract run and FHIR Agent Safety Eval must pass and be recorded in [docs/adapters.md](./adapters.md) before this row moves to a synthetic-evaluation tier. |
